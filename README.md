...
一覧は**カテゴリ別にまとめて**出す（並びはカテゴリ管理の順、`src/client/lib/glossary-groups.ts`）。
見出しで開閉でき、閉じたカテゴリは端末ごとに覚える（`studyrecall:glossary-closed-categories`）。
**検索語が入っている間は閉じたカテゴリも開いて出す**（閉じた中の一致が見えないと「該当なし」に見える）。
並べ方だけを変えていて、何が見えるか（検索・タグ・習得ステータス）は変えていない。
...
- 辞書からの生成は 1 回 10 件まで。範囲が 10 件を超えると、**まだ問題の無い用語を優先してランダムに**選ぶ
  （`src/client/lib/glossary-generate-targets.ts`）。先頭から切ると一覧が更新順なので同じ用語ばかり増える。
  選ぶのはボタンを押した瞬間（描画のたびに引くと再描画で対象が入れ替わる）
...
- **ポモドーロ**（タイマー）— 既定は 25分集中 / 5分休憩。**周期（集中・休憩・◯回ごとの長い休憩）を変えられ、全端末で共有される**。フェーズ切替でビープが鳴る
...
| チャンク                                   | サイズ               | 読み込まれる契機                         |
| ------------------------------------------ | -------------------- | ---------------------------------------- |
| `index`（＋ `cn` / `jsx-runtime`）         | 377 KB (gzip 117 KB) | 起動時                                   |
| 画面（ノート・タスク・辞書・復習・統計）   | 計 103 KB            | 起動後に裏で先読み（開けばその場で読む） |
| ダイアログ（連携設定・カテゴリ管理など 7） | 計 30 KB             | 同上                                     |
| `MarkdownRenderer`                         | 158 KB (gzip 46 KB)  | ノートをプレビュー表示したとき           |
| `jszip`                                    | 96 KB (gzip 28 KB)   | ZIP を書き出すとき                       |
| `confetti`                                 | 11 KB (gzip 4 KB)    | 紙吹雪を出すとき                         |

起動直後に見えるのはタイマー画面（か前回の画面 1 つ）なので、ほかの画面・Markdown・ZIP・紙吹雪はその時点では要らない。

- 分割前は単一チャンク 609 KB（gzip 189 KB）で、Vite の 500KB 警告が出続けていた。
- v1.9.0 までは画面とダイアログも `index` に入っていた（505 KB、gzip 148 KB）。
- **画面は `lazyWithPreload`**（`src/client/lib/lazy-with-preload.tsx`）で外す。
  `React.lazy` だけだと、読み込み済みでも初回の描画で一瞬 suspend して「読み込み中」が挟まる。
  先読みしたものは lazy を通さず直接描く。
- **ダイアログは一度開くまで描かない**（`useOpenedOnce`）。描いた時点で読み込みが始まるため。
  一度開いたら今までどおりマウントしたままにする（閉じるたびに外すと、中で読み込んだ一覧が毎回消える）。
- `StudyTab` は既定の画面で、タイマーの本体も抱えているので最初から載せている。

**Worker は圧縮してビルドする**（`vite.config.ts` の `environments.study_recall.build.minify`）。
既定では圧縮されず 2.2 MB（gzip 445 KB）あり、しばらく使われなかったあとの最初の応答でこれを読み込んでいた。
...
- **カテゴリは使用中だと削除できない**。消すと学習記録・ノート・問題まで cascade で失われるため、
  件数を示して拒否する。
- **ポモドーロのフェーズは `elapsedMs` と周期から導出する**。サーバーに持たせているのは `mode` と、
  **開始した時点の周期**（`timer_sessions` の 4 列）だけ。全端末が同じ経過時間と周期を見るので、
  フェーズも残り時間もアラームのタイミングも自動的に揃う。
- **周期の設定は全端末で共有し、開始時にセッションへ写す**（`user_settings` → `timer_sessions`）。
  端末ごとに持つと同じセッションの表示がずれ、セッションに写さないと走行中に設定を変えたときフェーズが飛ぶ。
  既定値は以前の固定値（25/5・長い休憩なし）なので、設定していない人の動きは変わらない（`src/shared/pomodoro-config.ts`）。
- **ポモドーロで記録する学習時間は休憩（長い休憩も）を除いた集中時間**。25分×2 + 休憩5分 = 経過55分なら 50分を記録する
