...
export default function MoveNoteDialog({
      ...
      return (
        <li key={node.id}>
          ...
          >
            ...
            {isCurrentParent && (
              <span className="shrink-0 text-xs text-slate-400 dark:text-slate-500">（現在）</span>
            )}
          </button>
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="ノートの移動先を選ぶ"
      className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-4 sm:items-center dark:bg-slate-950/70"
    >
      <div className="max-h-full w-full max-w-md overflow-y-auto rounded-2xl bg-white shadow-xl dark:bg-slate-900">
              ...
              <section key={category.id}>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isCurrentRoot
                      ? 'cursor-not-allowed text-slate-300 dark:text-slate-600'
                      : 'font-medium text-slate-700 hover:bg-blue-50 dark:text-slate-300 dark:hover:bg-blue-950',
                  )}
                >
                  <FolderTree className="h-3.5 w-3.5 shrink-0" aria-hidden />
                  このカテゴリの直下へ
                  {isCurrentRoot && (
                    <span className="text-xs text-slate-400 dark:text-slate-500">（現在）</span>
                  )}
                </button>
